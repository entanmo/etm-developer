...
本章将会讲解多个实际案例，由浅及深，由易到难的为开发者剖析DApp的全过程。
如果开发者对dapp还不甚了解 ，请查看合约开发章节,该章节讲解了合约模版各个文件的作用，以及开发者需要关注的模块。
...
很显然，可以看到注册的接口（并不是合约），细节可以参考远程接口章节。
示例：这里演示一个带参数的put请求
...
综上两种方式，我们可以看到，合约的请求分为两种，**一种是put请求（一般定义在contract目录下），一种是get请求（一般定义在interface目录下）**。


#### 1.2 模型定义
模型的定义其实是定义数据结构，让数据以什么方式存储到区块链上。在dapp模版中，我们可以看到model目录下对应的[words.js](https://github.com/etm-dev/etm-doc/tree/master/example/helloworld/model/words.js)就是对于的数据模型的定义文件。
...
可以发现，本例定义了一张words表，包含words字段，以及要求。

#### 1.3 合约实现
entanmo官方为了让开发者更容易的开发合约，在设计上就是尽量让开发者只做逻辑相关的工作。我们来看一下contract目录下的[helloworld.js](https://github.com/etm-dev/etm-doc/tree/master/example/helloworld/contract/helloworld.js)实现
...
注释中已经说明代码含义。开发者只需要关注合约中的逻辑实现即可。

写好逻辑实现后重启应用节点即可，dapp就编写完成了。
...
-----------------
### 3.秘密dapp
区块链技术是基于密码学的基础上设计的，En-Tan-Mo的账号系统是基于ed25519算法的，这套算优势就是`签名和验证的性能都极高，签名小，公钥小`,可以说该套算法专为签名设计。
...
基于以上理论，entanmo为开发者给出官方示例（[secret](https://github.com/etm-dev/secretDapp)）
